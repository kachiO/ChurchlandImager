...
%      stop.  All inputs are passed to IntrinsicImager_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 06-May-2014 18:30:43
...
global imagerhandles IntrinsicFlag digitalOut
...
if isempty(IntrinsicFlag)
    ...
    %            disp('daq already initialized exist and not empty. sproceed.')
    handles.frameRate =str2double(get(findobj('Tag','FrameRate'),'String'));   %this will be set to 10 fps
    handles.AcquireTime = str2double(get(findobj('Tag','AcquireTime'),'String'));
    framesPerTrigger = updateFramesPerTrigger(handles.frameRate,handles.AcquireTime);
...
handles.framesPerTrigger = framesPerTrigger;
set(findobj('Tag','framesPerTrigger'),'String',handles.framesPerTrigger);
set(handles.vidObj,'FramesPerTrigger',handles.framesPerTrigger);

handles.BlueLEDState = 1;
handles.GreenLEDState = 0;
handles.RedLEDState = 0;
handles.AllLEDState = [handles.GreenLEDState handles.RedLEDState handles.BlueLEDState];
...
hImage = image(zeros(imHeight,imWidth,nbands),'parent',handles.previewer);
%     hImage = imshow(zeros(imHeight,imWidth,nbands),[],'parent',handles.previewer);
% preview(handles.vidObj,hImage)
start(handles.vidObj);

handles.hImage = hImage;
histax = handles.histogram;
prevax = handles.previewer;
setappdata(hImage,'UpdatePreviewWindowFcn',@histogramDisplay); %call histogram display function
...
% %     %histogram update callback function using preview
    function histogramDisplay(obj,event,hImage)
        nbins = 128;
        t = 40;
        ...
        set(hImage,'CData',Image); %continue to display live feed
        ...
        val = get(handles.histogramlist,'value');
...
if handles.vidObj == 0
    ...
    dir = [start_dir '\' date '\' animal '\' exptnum '\' ];
    if ~isdir(dir)
        mkdir(dir)
    end
...
% --- Executes on selection change in LoggingMenu.
function LoggingMenu_Callback(hObject, eventdata, handles)
% hObject    handle to LoggingMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns LoggingMenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from LoggingMenu

index = get(hObject,'value');  %get index of current user selection
modes =  propinfo(handles.vidObj,'LoggingMode'); %determine possible logging modes
LoggingMode = modes.ConstraintValue{index}; %select
set(handles.vidObj,'LoggingMode',LoggingMode);

handles.LoggingMode  = LoggingMode;
...
% --- Executes during object creation, after setting all properties.
function LoggingMenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to LoggingMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in TriggerMenu.
function TriggerMenu_Callback(hObject, eventdata, handles)
% hObject    handle to TriggerMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns TriggerMenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from TriggerMenu

selection = get(hObject,'value'); %get handle for user selected value from menu
config = triggerinfo(handles.vidObj);
triggerconfig(handles.vidObj,config(selection)); %set trigger mode

guidata(hObject,handles);
end

% --- Executes during object creation, after setting all properties.
function TriggerMenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to TriggerMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%(22-jan-2013) intrinsic frame rate hard-coded to 10fps. frame rate can't
%cant be changed from matlab. Has to be set in PFRemote software. I think
%the exposure can be adjusted though. For 10 fps between 0.010 ms - 72.5
%ms. But I'd rather not mess with this via matlab.
...
handles.AcquireTime = str2double(get(hObject,'String'));
handles.frameRate =str2double(get(findobj('Tag','FrameRate'),'String'));
...
% hObject    handle to framesPerTrigger (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of framesPerTrigger as text
%        str2double(get(hObject,'String')) returns contents of framesPerTrigger as a double
...
% --- Executes on button press in blueLedon.
function blueLedon_Callback(hObject, eventdata, handles)
% hObject    handle to blueLedon (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global digitalOut
...
AllLEDState  = [greenLEDstate redLEDstate (~blueLEDstate)];
...
handles.AllLEDState = [get(findobj('Tag','greenFlag'),'Value') get(findobj('Tag','redFlag'),'Value') ~(get(findobj('Tag','blueFlag'),'Value'))];
...
% handles    structure with handles and user data (see GUIDATA)
 ...
 IntrinsicImagerGUI_CloseRequestFcn(hObject, eventdata, handles)
